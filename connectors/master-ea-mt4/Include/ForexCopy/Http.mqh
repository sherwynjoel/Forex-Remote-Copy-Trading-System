...
struct RetryItem
  {
   ...
   // latter is the *symbol's* last known tick time, which silently stalls
   // whenever no new price ticks arrive (thin demo liquidity, market
   // closed, terminal just attached). Retry scheduling must keep moving
   // regardless of whether prices are moving.
